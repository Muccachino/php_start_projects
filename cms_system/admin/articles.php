    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 max-w-xl mb-10">
        ...
        <?php foreach ($articles as $article) : ?>
            <tr class="bg-white border-b dark:bg-gray-800">
                ...
                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap"><?= e(format_date($article["created"])) ?></td>
                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap"><?= e($article["category"]) ?></td>
                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap"><?= e($article["published"]) ? "Yes" : "No" ?></td>
                <td class="px-6 py-4 font-medium text-pink-600 whitespace-nowrap"><a
                            href="article.php?id=<?= e($article["id"]) ?>">Edit</a></td>
                <td class="px-6 py-4 font-medium text-blue-600 whitespace-nowrap"><a
                            href="article-delete.php?id=<?= e($article["id"]) ?>">Delete</a></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
